#!/usr/bin/env tsx
/**
 * Liechtenstein Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://www.gesetze.li';
const PORTAL_NAME = 'Liechtenstein Law Portal';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/liechtenstein-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Liechtenstein Law MCP — Data Freshness Check');
