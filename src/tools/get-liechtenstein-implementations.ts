/**
 * get_liechtenstein_implementations — Find Liechtenstein statutes that reference a specific EU directive/regulation.
 */

import type Database from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetLiechtensteinImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface LiechtensteinImplementationResult {
  document_id: string;
...
export async function getLiechtensteinImplementations(
  db: InstanceType<typeof Database>,
  input: GetLiechtensteinImplementationsInput,
): Promise<ToolResponse<LiechtensteinImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as LiechtensteinImplementationResult[];
