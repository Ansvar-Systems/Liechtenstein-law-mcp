/**
 * search_eu_implementations — Search EU directives/regulations referenced by Liechtenstein legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_liechtenstein_implementation?: boolean;
  limit?: number;
}
...
export interface EUImplementationSearchResult {
  ...
  liechtenstein_statute_count: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as liechtenstein_statute_count
  ...
  if (input.has_liechtenstein_implementation) {
    sql += ' HAVING liechtenstein_statute_count > 0';
  }
