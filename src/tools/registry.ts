/**
 * Tool registry for Liechtenstein Law MCP Server.
...
import { getLiechtensteinImplementations, type GetLiechtensteinImplementationsInput } from './get-liechtenstein-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including the Landesgesetzblatt (Official Gazette of Liechtenstein). ' +
    ...
    'Call this FIRST when you need to understand what Liechtenstein legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Liechtenstein statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    name: 'get_provision',
    description:
      'Retrieve the full text of a specific provision (section) from an Liechtenstein statute. ' +
  ...
  },
  {
    ...
    description:
      'Validate an Liechtenstein legal citation against the database — zero-hallucination check. ' +
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Liechtenstein statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Liechtenstein?" ' +
  ...
  },
  {
    ...
    description:
      'Format an Liechtenstein legal citation per standard conventions. ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether an Liechtenstein statute or provision is currently in force, amended, repealed, or not yet in force. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get the EU legal basis that an Liechtenstein statute references or aligns with. ' +
      'Liechtenstein is an EEA member state and Liechtenstein laws implement EU directives/regulations ' +
      'where applicable. ' +
      'Returns EU document identifiers, reference types, and implementation status.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Liechtenstein statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_liechtenstein_implementations',
    description:
      'Find all Liechtenstein statutes that reference or align with a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Liechtenstein statutes. ' +
      'Liechtenstein transposes EU directives into national law as an EEA Member State.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that are referenced by Liechtenstein legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_liechtenstein_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents referenced by Liechtenstein legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU legal basis for a SPECIFIC provision within an Liechtenstein statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU alignment status for an Liechtenstein statute or provision. ' +
      ...
      'Liechtenstein is an EU Member State; this checks implementation status and cross-references.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Liechtenstein statute identifier.' },
...
const EU_TOOL_NAMES = new Set([
  'get_eu_basis',
  'get_liechtenstein_implementations',
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_liechtenstein_implementations':
          result = await getLiechtensteinImplementations(db, args as unknown as GetLiechtensteinImplementationsInput);
          break;
