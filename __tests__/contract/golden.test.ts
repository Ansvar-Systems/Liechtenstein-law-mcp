...
describe('Negative tests', () => {
  ...
  it('should return no results for invalid section', () => {
    const row = db.prepare(
      "SELECT COUNT(*) as cnt FROM legal_provisions WHERE document_id = 'li-cybersicherheitsgesetz' AND section = '999ZZZ-INVALID'"
    ).get() as { cnt: number };
    expect(row.cnt).toBe(0);
  });
});
...
describe('All 10 laws are present', () => {
  const expectedDocs = [
    ...
    'li-e-commerce-gesetz',
    'li-e-government-gesetz',
    'li-fmag-financial-market',
    'li-kommunikationsgesetz',
    'li-signaturgesetz',
